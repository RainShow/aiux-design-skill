...
**列表已挂 create / detail**：不要用无返回的 `PlaceholderPage` 顶路由。创建走表单模板（顶栏返回），详情走详情模板（顶栏返回）；返回读 `state.from`，缺省回列表。

**演示 CRUD 数据（可选）**：纯前端演示、用户需要刷新后仍保留增删改时，模块 demo store 可用 `sessionStorage`（写失败则内存兜底），与登录态 `writeDemoAuthed` 同思路。这是业务仓演示约定，不是页面视觉规范；不要为此改 `*-page.md` 结构。
